...
resource "aws_api_gateway_deployment" "hackathon_deployment" {
  depends_on = [
    ...
    aws_api_gateway_integration.users_any_integration,
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_integration.users_any_integration.id,
      aws_api_gateway_integration.user_get_integration.id,
...
resource "aws_api_gateway_integration" "users_any_integration" {
  rest_api_id = aws_api_gateway_rest_api.hackathon_api.id
  resource_id = aws_api_gateway_resource.users_resource.id
